...
function deleteImage(imageID){
	var x = 'http://userstory.io/image/'+imageID+'delete'; 
	console.log('final url',x);
	$.ajax({
		url: 'http://userstory.io/image/'+imageID+'delete',
		type: 'get',
		processData:false,
		contentType: false,
		cache: false,
		...
		if(status == 'success')
			console.log('image with imageID',imageID, 'deleted');
		else{
...
$(function(){
	...
	$('.copylink').on('click',function (e){
		var $temp = $("<input>");
		$("body").append($temp);
		$temp.val(document.getElementById("imageurl").value).select();
	...
	//on saving image, previous image is deleted
	$('.saveimage').on('click',function (e){
		var x=staticPlugin.getPluginByKey('userstory')
		editor.createLastCanvas('toolbar',function (data){
			x.run(data, e,function(imageURL){
				var imageid = $('#imageurl').val().replace(/\n/g,'');
				console.log('imageURL ...to be deleted',imageid);
				deleteImage(imageid.match(/(?:image\/)(.*)/)[1]);
				document.getElementById("imageurl").value = imageURL;
			});
		});
	});
